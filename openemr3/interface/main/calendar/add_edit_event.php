        ...
        if (!empty($_POST['form_pid'])) {
            ...
            if ((int)$GLOBALS['auto_create_new_encounters']
                ...
                if (isset($GLOBALS['temporary-eid-for-manage-tracker']) || !empty($_GET['eid'])) {
                    ...
                    manage_tracker_status($event_date, $appttime, $temp_eid, $_POST['form_pid'], $_SESSION["authUser"], $_POST['form_apptstatus'], $_POST['form_room'],$_POST['appointment_type'], $encounter);
                }
            } else {
                # Capture the appt status and room number for patient tracker.
                if (!empty($_GET['eid'])) {
                    manage_tracker_status($event_date, $appttime, $_GET['eid'], $_POST['form_pid'], $_SESSION["authUser"], $_POST['form_apptstatus'], $_POST['form_room'], $_POST['appointment_type'],$is_tracker);
                }
            }
        }
            ...
            if ($GLOBALS['select_multi_providers'] && $row['pc_multiple']) {
                ...
                if ($_POST['recurr_affect'] == 'current') {
                    ...
                    foreach ($_POST['form_provider'] as $provider) {
                        ...
                        "appointment_type = '" . add_escape_custom($_POST['appointment_type']) . "', "  .
...
*********************************************************************/
 ...
 // If we are editing an existing event, then get its data.
    if ($eid) {
        ...
        $pcroom = $row['pc_room'];
        $_POST['appointment_type'] = $row['appointment_type'];
...
<?php require($GLOBALS['srcdir'] . "/restoreSession.php"); ?>
 ...
 function set_display() {
  ...
  if (s.selectedIndex >= 0) {
   ...
   var appointment_type = document.getElementById('appointment_type').style;
 ...
 function dateChanged() {
  ...
  if (si < opts.length) f.form_repeat_type.selectedIndex = si;
    ...
    function find_available(extra) {
        ...
        var at=document.forms[0].appointment_type;
        console.log(at);
  ...
  <td nowrap id='tdallday5'>
   <input class='input-sm'    type='text' size='4' name='form_duration' value='<?php echo attr($thisduration) ?>' title='<?php echo xla('Event duration in minutes'); ?>' />
    ...
    <tr>
        ...
        <tr>
        <td nowrap><b><?php echo xlt('Appointment'); ?>:</b></td>
        <?php echo $row['appointment_type']?>
        <td>

        <select class="form-control form-control-lg" name="appointment_type" id="appointment_type" >
        <option value=" "><?php echo $row['appointment_type']?></option>
